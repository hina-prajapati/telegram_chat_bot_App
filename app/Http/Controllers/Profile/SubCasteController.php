...
class SubCasteController extends BaseQuestionController
{
    public function handle($chatId, $text, TelegramUserState $state)
    {
        ...
        if (empty($answers['caste'])) {
            return [
                'text' => "❗ Please select your Caste first before choosing a Sub-Caste.",
                'halt_flow' => true
            ];
        }

        $validSubCastes = $this->getSubCasteList($answers['caste']);
        ...
        // ✅ Save selection
        $answers['sub_caste'] = $text;
        $state->answers = $answers;
        $state->save();

        $this->saveAnswer($chatId, $state, 'sub_caste', $text, Profile::class);
    ...
    {
        return __('messages.ask_sub_caste'); // Add this key in your lang file
    }
    ...
    public static function getOptions(array $answers = []): array
    {
        if (empty($answers['caste'])) {
            return [
                'text' => '❗ Please select a Caste first.',
                'options' => [],
                'halt_flow' => true
            ];
        }

        $subCastes = (new self())->getSubCasteList($answers['caste']);
